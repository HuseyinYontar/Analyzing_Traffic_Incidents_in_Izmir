...
# =========================
file_path = "izbb-kaza-ariza-verileri-SON-binned-updated.xlsx" # uses your helper
...
# Fit transform once; turn into dense for DecisionTree + SFS
...
def evaluate_k(k: int):
    sfs = SequentialFeatureSelector(
        ...
        direction="forward",      # or "backward" if you want
        scoring="accuracy",
        cv=5,
        n_jobs=-1,
    )
    ...
    # Refit a model on ONLY those features and do CV to score
...
# 5) SWEEP k = 1..max_k
# =========================
max_k = min(len(all_feature_names), 50)  # cap to 50 to keep it reasonable
scores = []
feature_counts = []
...
with open("best_features_sfs.txt", "w", encoding="utf-8") as f:
...
plt.tight_layout()
plt.savefig("sfs_decision_tree_accuracy_curve.pdf")
plt.show()
