...
# =========================
# 3. RANDOMLY SAMPLE 100 ROWS
# =========================
df = df.sample(random_state=42).reset_index(drop=True)
...
# =========================
feature_range = range(1,  X_train_encoded.shape[1] )  # up to 14 or less if limited
accuracies = []
...
for n_features in feature_range:
    ...
    X_selected = X_train_encoded[:, sfs.get_support()]
...
plt.plot(feature_range, accuracies, 'bo-', linewidth=2)
plt.title("Sequential Feature Selection Performance (100-row sample)", fontsize=14)
...
plt.scatter(feature_range[best_idx], accuracies[best_idx], color='red', s=80, label="Best")
plt.legend()
plt.tight_layout()
plt.show()
...
# =========================
best_features = feature_range[best_idx]
best_accuracy = accuracies[best_idx]
best_sfs = sfs_objects[best_idx]  # get SFS object for best run

print(f"\n✅ Best number of features: {best_features}")
