...
# =========================================================
# 5) Agglomerative clustering via SciPy linkage + fcluster
# =========================================================
def agglomerative_labels_from_linkage(Z, n_clusters: int):
    """
    Returns labels 0..k-1 using maxclust criterion.
    """
    labs = fcluster(Z, t=int(n_clusters), criterion="maxclust")
    return labs.astype(int) - 1  # 0-based


# =========================================================
# 6) Dendrogram (plot + cut line for k)
# =========================================================
def plot_dendrogram_figure(Z, ilce_labels, k, title, truncate=True, p=30):
    dists = Z[:, 2]
    n = len(ilce_labels)
    target_merges = n - k
...
# =========================================================
# 7) Cluster plots: EXACTLY 2 graphs + 1 heatmap
# =========================================================
...
def cluster_heatmap(center_series, day_order, hours, types, title):
    cube = center_series.unstack(level=-1)  # columns type, index (day,hour)
    ...
    for i, day in enumerate(day_order):
        if day in cube.index.get_level_values(0):
            mat_day = cube.loc[day].reindex(index=hours, columns=types).fillna(0.0)
            heat[i, :] = mat_day.sum(axis=1).values  # sum over types -> per hour
    ...
    plt.colorbar()
    plt.title(title + " – Heatmap (Day×Hour, summed over KAZA_TIPI)")
    plt.tight_layout()
    return fig
...
def main():
    ...
    LINKAGE_METHOD = "average"   # "average", "complete", "ward", "single"
    METRIC = "euclidean"         # keep euclidean for dendrogram + interpretability
    # ===============================================
    ...
    # ---- Build linkage (this IS agglomerative hierarchy)
    Z = linkage(X, method=LINKAGE_METHOD, metric=METRIC)

    # ---- Cut tree into N_CLUSTERS
    labels_arr = agglomerative_labels_from_linkage(Z, N_CLUSTERS)
    ...
    # cluster centers in original count cube (DAY,HOUR,TYPE)
    centers_counts = pivot_counts.copy()
    centers_counts["cluster"] = labels.values
    centers_counts = centers_counts.groupby("cluster").mean()

    # ---------- PDF OUTPUT ----------
    pdf_name = "AGG_day_hour_kazatype_dendrogram_and_profiles_clean.pdf"
    with PdfPages(pdf_name) as pdf:
        fig_d = plot_dendrogram_figure(
            Z,
            ilce_labels=features.index.tolist(),
            k=N_CLUSTERS,
            title=f"Dendrogram (Agglomerative) – method={LINKAGE_METHOD}, metric={METRIC}",
            truncate=True,
            p=min(30, X.shape[0]),
        )
        pdf.savefig(fig_d)
        ...
        # per cluster: 2 graphs + heatmap
        for cl in centers_counts.index:
            center_series = centers_counts.loc[cl]  # MultiIndex Series (DAY,HOUR,TYPE)
            ...
            fig_hm = cluster_heatmap(center_series, day_order, hours, types, title=f"Cluster {cl}")
            pdf.savefig(fig_hm)
            plt.show()
    ...
    out_features.to_excel("AGG_day_hour_kazatype_features_clusters.xlsx")

    labels.reset_index().rename(columns={"index": "ILCE"}).to_excel(
        "AGG_day_hour_kazatype_ilce_clusters.xlsx", index=False
    )

    centers_counts.to_excel("AGG_day_hour_kazatype_cluster_centers_counts.xlsx")
    pivot_counts.to_excel("AGG_day_hour_kazatype_ilce_day_hour_type_counts.xlsx")

    print("\nSaved:")
    print(f" - {pdf_name}")
    print(" - AGG_day_hour_kazatype_features_clusters.xlsx")
    print(" - AGG_day_hour_kazatype_ilce_clusters.xlsx")
    print(" - AGG_day_hour_kazatype_cluster_centers_counts.xlsx")
    print(" - AGG_day_hour_kazatype_ilce_day_hour_type_counts.xlsx")
